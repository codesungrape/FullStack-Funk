...
// Mock TextEncoder/TextDecoder which aren't available in Node.js environment by default
...
const mockFetchResponse = (
      ...
      body: contentType.includes('text/event-stream')
        ? {
            getReader: () => {
              let callCount = 0;
              const streamData = [
                { done: false, value: new TextEncoder().encode('data: {"lyrics":"Test lyrics part 1"}\n\n') },
                { done: false, value: new TextEncoder().encode('data: {"lyrics":" - continued"}\n\n') },
  ...
  describe('Sidebar Component', () => {
    ...
    test('renders sidebar with all sections', () => {
        ...
        expect(screen.getByText("Let's Create Your Tech Tunes")).toBeInTheDocument();
      ...
      test('allows user to input study notes', async () => {
        ...
        const textarea = screen.getByPlaceholderText('Add your study notes here...');
      ...
      test('handles streaming response correctly', async () => {
        ...
        await waitFor(() => {
          expect(screen.getByText('Generated Lyrics:')).toBeInTheDocument();
          expect(screen.getByText('Test lyrics part 1 - continued')).toBeInTheDocument();
        });
      });
      ...
      test('handles error response correctly', async () => {
        ...
        await waitFor(() => {
          expect(screen.getByText('Failed to generate lyrics. Please try again.')).toBeInTheDocument();
        });
      });
